...
fn bench_bicontiguous_find_containment(c: &mut Criterion) {
    use netip::{BiContiguous, ipv6_bicontiguous_find_containment};
    ...
    // Builds `count` distinct networks spread evenly across `shape_count`
    // deep `(hi_prefix, lo_prefix)` shapes (each in `[20, 63]`, never `[0,
    // 19]`), with a distinct address per element. At this depth the expected
    // number of accidental cross-shape containments across the whole fixture
    // (`~ count^2 * 2^-40`) stays negligible even at `count = 8192`, so this
    // is the "never-contained" case: the honest worst case for both paths,
    // since neither ever gets to early-exit on a real containment hit.
    fn never_contained(count: u64, shape_count: u64) -> Vec<BiContiguous<Ipv6Network>> {
        (0..count)
            .map(|index| {
    ...
    // Mirrors the maintainer's production masks: hi half /40 (a "site"), lo
    // half /32 or /16, shared across a small pool of sites -- realistic
    // shape count (`S = 2`), realistic near-zero real containment.
    ...
    // A handful of "root" rectangles, each covering many finer networks
    // nested inside it -- heavy, real containment throughout.
    fn heavy_containment(count: u64, root_count: u64) -> Vec<BiContiguous<Ipv6Network>> {
        let roots: Vec<(u64, u64)> = (0..root_count)
            .map(|root| {
                (
                    root.wrapping_mul(0x9E37_79B9_7F4A_7C15),
                    root.wrapping_mul(0xBF58_476D_1CE4_E5B9),
                )
            })
            .collect();
        (0..count)
            .map(|index| {
                let (root_hi, root_lo) = roots[(index % root_count) as usize];
                let depth = (20 + (index / root_count) % 40) as u8;
                let mask = shape_mask(depth, depth);
                let addr = ((root_hi as u128) << 64) | root_lo as u128;
                BiContiguous::try_from(Ipv6Network::from_bits(addr & mask, mask)).unwrap()
            })
            .collect()
    }
    ...
    for &(count, shape_count) in &[
        ...
        group.bench_function(
            format!("ipv6_bicontiguous_find_containment {count}x never-contained S={shape_count}"),
            |b| {
                let template = never_contained(count, shape_count);
                b.iter_batched(
                    || template.clone(),
                    |mut nets| {
                        core::hint::black_box(ipv6_bicontiguous_find_containment(&mut nets));
                    },
                    criterion::BatchSize::SmallInput,
                );
            },
    ...
    for &count in &[256u64, 1024, 8192] {
        group.throughput(Throughput::Elements(count));
        group.bench_function(format!("ipv6_bicontiguous_find_containment {count}x geo"), |b| {
            let template = geo(count);
            b.iter_batched(
                || template.clone(),
                |mut nets| {
                    core::hint::black_box(ipv6_bicontiguous_find_containment(&mut nets));
    ...
    for &count in &[256u64, 1024, 8192] {
        group.throughput(Throughput::Elements(count));
        group.bench_function(
            format!("ipv6_bicontiguous_find_containment {count}x heavy-containment"),
            |b| {
                let template = heavy_containment(count, 8);
                b.iter_batched(
                    || template.clone(),
                    |mut nets| {
                        core::hint::black_box(ipv6_bicontiguous_find_containment(&mut nets));
    ...
    // Router crossover: a never-contained fixture (both paths' honest worst
    // case) at N straddling `CONTAINMENT_THRESHOLD`, fine-grained enough to
    // see where the router's choice should flip.
    for &count in &[16u64, 32, 48, 64, 80, 88, 96, 104, 112, 128, 192, 256] {
        group.throughput(Throughput::Elements(count));
        group.bench_function(
            format!("ipv6_bicontiguous_find_containment {count}x never-contained crossover"),
            |b| {
                let template = never_contained(count, 4);
                b.iter_batched(
                    || template.clone(),
                    |mut nets| {
                        core::hint::black_box(ipv6_bicontiguous_find_containment(&mut nets));
...
criterion_group!(
    ...
    bench_aggregate,
    bench_bicontiguous_find_containment,
    bench_binary_split,
    bench_supernet_for,
    bench_difference,
