...
mod test {
    ...
    // Shared with `net::contiguous`'s test module (used by
    // `prop_bicontiguous_ipv6_from_contiguous_preserves_value` here and by
    // several `Contiguous`-only tests there), so it stays put instead of
    // moving alongside the other `arb_contiguous_*` strategies.
